...
from sql.sql_constants import CURR_AFFILIATES_QUERY, VERIFIED_AFFILS_QUERY
...
def output_to_tsv(affiliates):
    ...
    Returns:
        None
    """
    affiliates.to_csv('affiliate_std_name_scores.tsv', sep='\t', index=False)


def main():
    # the data being queried is currently only the NBER affiliates
    affiliates = pd.DataFrame(execute_sql(CURR_AFFILIATES_QUERY))
